-- Personal work-log helper: appends dated notes/case numbers to a text file
-- (M.add / M.add_by_day) and formats them into a weekly report
-- (M.generate / M.preview). Wired up to <leader>n* mappings in
-- lua/my_plugin/notes.lua.
local fn = vim.fn
...
local function get_previous_date_from_now(day)
  ...
  local days_from_now_in_hrs = 24 * days_from_now
...
local function parse_note_to_tbl()
  ...
  for line in file:lines() do
    if starts_with(line, M.note_starting_sym) then
      ...
      table.insert(note_content[day], note_split[2])
      ending_day = day
    end

    if starts_with(line, M.case_starting_sym) then
...
local function parse_note()
  ...
  for _, day in pairs(M.days_of_week) do
    local day_with_date = day .. '(' .. get_previous_date_from_now(day) .. ')'
    formatted_notes = formatted_notes .. '\n' .. day_with_date .. '\n'
    if notes[day] ~= nil then
      for _, note in pairs(notes[day]) do
        formatted_notes = formatted_notes .. '    - ' .. note .. '\n'
      end
    end
  end
...
function M.add_by_day()
  local day_input = fn.input()
  if day_input == nil or day_input == '' then
    return
  end
  local day_index = utils.find_index_by_value(M.days_of_week, day_input)
  if day_index == 0 then
    print('Invalid day')
  end
  M.add(day_input)
